# GPT-image-2.0 Brand Brief for AdaptOrch MCP

Use this prompt if generating raster social/README artwork with GPT-image-2.0 or an equivalent image model. The checked-in GitHub README uses SVG assets so it renders without external image hosting.
...
- README hero: `1280x520`
- Social preview: `1200x630`
- Square icon: `1024x1024`

## Negative Prompt

No humanoid robot, no stock office photo, no excessive glassmorphism, no neon cyberpunk city, no unreadable tiny text, no logos from other companies, no fake UI screenshots containing real secret values.
