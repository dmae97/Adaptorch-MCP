...
The key is never exposed as an MCP tool argument or JSON body field. It is sent only on run submission, redacted from errors, and omitted from status, artifact, usage, and catalog requests. Older engines without provider-credential support fail closed; source checkouts can install the current engine with `make engine-local ENGINE_PATH=../adaptorch`.
...
| `ADAPTORCH_MCP_PROVIDER_API_KEY_COMMAND` | Rotating BYOK credential. | Caller-owned command (no shell) printing only the access token; resolved once per submission, never for polling or redaction. |
| `ADAPTORCH_MCP_PROVIDER_AUTH_TYPE` | `api_key` or `oauth`. | OAuth currently supports explicit `openai_codex`. Both server and local engine need matching source support. |
| `ADAPTORCH_MCP_PROVIDER_ACCOUNT_ID` | Request-scoped OAuth account selection. | Forwarded as `X-Provider-Account-Id`, omitted from repr/errors; does not reuse the host account. |
