...
- Re-pinned the public engine lock from `22a4e388` to the latest published `b3bb2d90`, bringing the wrapper up to the current hosted control-plane contract (BYOK admission, verifier-gated selection availability, and judge-arbiter readiness). Working-tree MCP BYOK forwarding requires the accompanying local engine change; an older installed engine fails closed until that engine revision is published and the lock can advance again.
